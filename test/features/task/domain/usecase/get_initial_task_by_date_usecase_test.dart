...
void main() {
  ...
  test("Should returns ServerFailure that contains combine task entity as its data when getTaskByDate returns ServerFailure", () async {
    ...
    final CombineTaskEntity expectedResult = CombineTaskEntity(monthlyTaskEntities: mockedMonthlyTaskEntities, taskEntities: const []);

    when(mockAuthenticationRepository.getToken()).thenAnswer((_) async => const Right(AuthenticationEntity(token: Utils.token)));
    when(mockTaskRepository.getMonthlyTask(token: Utils.token, time: time)).thenAnswer((_) async => Right(mockedMonthlyTaskEntities));
    when(mockTaskRepository.getTaskByDate(token: Utils.token, time: time)).thenAnswer((_) async => const Left(ServerFailure(message: Failure.generalError, data: [])));
  ...
  test("Should returns ServerFailure that contains combine task entity as its data when getTaskByDate returns ServerFailure with data", () async {
    final time = DateTime(2022, 7, 16);
    final List<MonthlyTaskEntity> mockedMonthlyTaskEntities = [
      MonthlyTaskEntity(id: "001", date: time, total: 2),
      MonthlyTaskEntity(id: "002", date: time.add(const Duration(days: 1)), total: 1),
  ...
  test("Should returns CacheFailure that contains combine task entity as its data when getTaskByDate returns CacheFailure", () async {
    ...
    when(mockAuthenticationRepository.getToken()).thenAnswer((_) async => const Right(AuthenticationEntity(token: Utils.token)));
    when(mockTaskRepository.getMonthlyTask(token: Utils.token, time: time)).thenAnswer((_) async => Right(mockedMonthlyTaskEntities));
    when(mockTaskRepository.getTaskByDate(token: Utils.token, time: time)).thenAnswer((_) async => const Left(CacheFailure(message: Failure.cacheError, data: [])));
  ...
  test("Should returns CacheFailure that contains combine task entity as its data when getTaskByDate returns CacheFailure with data", () async {
    final time = DateTime(2022, 7, 16);
    final List<MonthlyTaskEntity> mockedMonthlyTaskEntities = [
      MonthlyTaskEntity(id: "001", date: time, total: 2),
      MonthlyTaskEntity(id: "002", date: time.add(const Duration(days: 1)), total: 1),
  ...
  test("Should returns ServerFailure that contains combine task entity as its data when getMonthlyTask returns ServerFailure", () async {
    ...
    final CombineTaskEntity expectedResult = CombineTaskEntity(monthlyTaskEntities: const [], taskEntities: mockedTaskEntities);

    when(mockAuthenticationRepository.getToken()).thenAnswer((_) async => const Right(AuthenticationEntity(token: Utils.token)));
    when(mockTaskRepository.getMonthlyTask(token: Utils.token, time: time)).thenAnswer((_) async => const Left(ServerFailure(message: Failure.generalError, data: [])));
    when(mockTaskRepository.getTaskByDate(token: Utils.token, time: time)).thenAnswer((_) async => Right(mockedTaskEntities));
  ...
  test("Should returns ServerFailure that contains combine task entity as its data when getMonthlyTask returns ServerFailure with data", () async {
    final time = DateTime(2022, 7, 16);
    final List<MonthlyTaskEntity> mockedMonthlyTaskEntities = [
      MonthlyTaskEntity(id: "001", date: time, total: 2),
      MonthlyTaskEntity(id: "002", date: time.add(const Duration(days: 1)), total: 1),
  ...
  test("Should returns CacheFailure that contains combine task entity as its data when getMonthlyTask returns CacheFailure", () async {
    ...
    when(mockAuthenticationRepository.getToken()).thenAnswer((_) async => const Right(AuthenticationEntity(token: Utils.token)));
    when(mockTaskRepository.getMonthlyTask(token: Utils.token, time: time)).thenAnswer((_) async => const Left(CacheFailure(message: Failure.cacheError, data: [])));
    when(mockTaskRepository.getTaskByDate(token: Utils.token, time: time)).thenAnswer((_) async => Right(mockedTaskEntities));
  ...
  test("Should returns CacheFailure that contains combine task entity as its data when getMonthlyTask returns CacheFailure with data", () async {
    final time = DateTime(2022, 7, 16);
    final List<MonthlyTaskEntity> mockedMonthlyTaskEntities = [
      MonthlyTaskEntity(id: "001", date: time, total: 2),
      MonthlyTaskEntity(id: "002", date: time.add(const Duration(days: 1)), total: 1),
  ...
  test("Should returns UnexpectedFailure that contains combine task entity as its data when getMonthlyTask and getTaskByDate returns Failure", () async {
    final time = DateTime(2022, 7, 16);
    const CombineTaskEntity expectedResult = CombineTaskEntity(monthlyTaskEntities: [], taskEntities: []);
    ...
    expect(result, const Left(UnexpectedFailure(message: Failure.generalError, data: expectedResult)));
    verify(mockAuthenticationRepository.getToken());
    verify(mockTaskRepository.getTaskByDate(token: Utils.token, time: time));
    verify(mockTaskRepository.getMonthlyTask(token: Utils.token, time: time));
  });

  test("Should returns UnexpectedFailure that contains combine task entity as its data when getMonthlyTask and getTaskByDate returns Failure with data", () async {
    final time = DateTime(2022, 7, 16);
    final List<MonthlyTaskEntity> mockedMonthlyTaskEntities = [
      MonthlyTaskEntity(id: "001", date: time, total: 2),
      MonthlyTaskEntity(id: "002", date: time.add(const Duration(days: 1)), total: 1),
    ...
    expect(result, Left(UnexpectedFailure(message: Failure.generalError, data: expectedResult)));
    verify(mockAuthenticationRepository.getToken());
    verify(mockTaskRepository.getTaskByDate(token: Utils.token, time: time));
    verify(mockTaskRepository.getMonthlyTask(token: Utils.token, time: time));
  });
