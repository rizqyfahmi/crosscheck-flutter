...
void main() {
  ...
  test("Should returns CountedDailyTaskResponseModel properly", () async {
    final time = DateTime(2022, 7);
    final expected = [
      CountedDailyTaskModel(id: "20220701", date: DateTime.parse("2022-07-01"), total: 1),
      CountedDailyTaskModel(id: "20220702", date: DateTime.parse("2022-07-02"), total: 2),
      CountedDailyTaskModel(id: "20220703", date: DateTime.parse("2022-07-03"), total: 3),
      CountedDailyTaskModel(id: "20220704", date: DateTime.parse("2022-07-04"), total: 4),
      CountedDailyTaskModel(id: "20220705", date: DateTime.parse("2022-07-05"), total: 5),
    ];
    final response = stringify("test/features/task/data/datasource/task_remote_data_source_success_counted_daily_task_response.json");
    when(mockClient.get(any, headers: Utils().headers)).thenAnswer((_) async => Response(response, 200));

    final result = await taskRemoteDataSource.countDailyTask(token: Utils.token, time: time);

    expect(result, CountedDailyTaskResponseModel(message: Utils.successMessage, data: expected));
    verify(mockClient.get(any, headers: Utils().headers));
  });

  test("Should returns CountedDailyTaskResponseModel with empty list of data when data in response is null", () async {
    final time = DateTime(2022, 7);
    final expected = [];
    final response = stringify("test/features/task/data/datasource/task_remote_data_source_success_null_data_response.json");
    when(mockClient.get(any, headers: Utils().headers)).thenAnswer((_) async => Response(response, 200));
  ...
  test("Should returns CountedDailyTaskResponseModel with empty list of data when data in response contains empty list", () async {
    final time = DateTime(2022, 7);
    final expected = [];
    final response = stringify("test/features/task/data/datasource/task_remote_data_source_success_empty_data_response.json");
    when(mockClient.get(any, headers: Utils().headers)).thenAnswer((_) async => Response(response, 200));
  ...
  test("Should returns ServerException when count daily task by month from client is failed", () async {
    final time = DateTime(2022, 7);
    when(mockClient.get(any, headers: Utils().headers)).thenAnswer((_) async => Response(json.encode({"message": Failure.generalError}), 500));
